...
(function(context, namespace) {
  ...
  HopscotchBubble.prototype = {
    ...
    show: function() {
      var self      = this,
          ...
          fadeDur   = hopscotch.noFade?0:1000;

      utils.removeClass(this.element, 'hide');
      if(!hopscotch.noFade) utils.addClass(this.element, fadeClass);
      setTimeout(function() {
  ...
  Hopscotch = function(initOptions) {
    ...
    showStepHelper = function(stepNum) {
      ...
      if (!hopscotch.noFade) bubble.hide(false);
    ...
    this.startTour = function(tour, stepNum) {
      ...
      findStartingStep(currStepNum, function(stepNum) {
        ...
        if(!hopscotch.noFade) bubble.hide(false); // make invisible for boundingRect calculations when opt.animate == true
    ...
    this.endTour = function(clearState, doCallbacks) {
      ...
      if(!hopscotch.noFade) bubble.hide();
      if (clearState) {
